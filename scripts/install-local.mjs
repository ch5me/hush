...
`;
  ...
  // Delivery is done above: the launcher names the runtime this run just published. Everything
  // below is cleanup, not delivery, and runs after delivery so it can never block it -- an older
  // runtime that fails to prune just stays for the next run to retry.
