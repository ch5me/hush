...
function formatProvenance(node: HushResolvedNode): string[] {
  ...
  if (node.interpolation?.dependencies.length) {
    for (const dependency of node.interpolation.dependencies) {
      lines.push(
        `      ${pc.dim(`interpolation=${dependency.path}${dependency.filePath ? ` <- ${dependency.filePath}` : ""}`)}`,
      );
    }
  }
...
function formatConflict(conflict: HushBundleConflictDetail): string[] {
  return [
    ...
    ...conflict.contenders.map(
      (contender) =>
        `    ${pc.dim(`file=${contender.filePath} bundle=${contender.bundle ?? "-"}`)}`,
    ),
  ];
}
...
function filterResolvedNodes<T extends Record<string, HushResolvedNode>>(
  nodes: T,
...
function toResolveCompactRecord(
  target: string,
  logicalPath: string,
...
export async function resolveCommand(ctx: HushContext, options: ResolveOptions): Promise<void> {
  ...
  if (!target) {
    ...
    ctx.logger.error(
      pc.dim(
        `Available targets: ${Object.keys(repository.manifest.targets ?? {}).join(", ") || "(none)"}`,
      ),
    );
  ...
  try {
    ...
    const filteredLogicalPaths = [
      ...Object.keys(filteredValues),
      ...Object.keys(filteredArtifacts),
    ].sort();
    const compactRecords: HushCompactRecord[] = [
      ...Object.entries(filteredValues).map(([logicalPath, node]) =>
        toResolveCompactRecord(resolution.target, logicalPath, node),
      ),
      ...Object.entries(filteredArtifacts).map(([logicalPath, node]) =>
        toResolveCompactRecord(resolution.target, logicalPath, node),
      ),
    ].sort(
      (left, right) => left.key.localeCompare(right.key) || left.source.localeCompare(right.source),
    );
    ...
    if (options.json) {
      writeJsonSuccess(
        ctx,
        "resolve",
        toSafeResolutionJson(resolution, target, options.only, shadowed),
      );
      return;
    }
    ...
    if (Object.keys(filteredValues).length === 0) {
      ...
      for (const [logicalPath, node] of Object.entries(filteredValues).sort(([left], [right]) =>
    ...
    if (Object.keys(filteredArtifacts).length === 0) {
      ...
      for (const [logicalPath, node] of Object.entries(filteredArtifacts).sort(([left], [right]) =>
        left.localeCompare(right),
      )) {
        const entry = node.entry;
        lines.push(
          `  ${logicalPath} ${"type" in entry ? pc.dim(`(${entry.type}:${entry.format})`) : ""}`.trimEnd(),
        );
        lines.push(...formatProvenance(node));
      }
    }
