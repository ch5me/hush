...
type DiffSelection = { kind: "target"; name: string } | { kind: "bundle"; name: string };
...
function summarizeNode(node: HushResolvedNode): ComparableNode {
  if ("type" in node.entry) {
    return {
      ...
      interpolation: (node.interpolation?.dependencies ?? []).map(
        (dependency) =>
...
function loadHistoricalRepository(
  ctx: HushContext,
  repository: HushV3Repository,
  store: StoreContext,
  ref: string,
  ...
  const repoPrefix = (
    typeof prefixOutput === "string" ? prefixOutput : prefixOutput.toString("utf-8")
  )
    .trim()
    .replace(/\/$/, "");
  ...
  const historicalFileIndex = Object.fromEntries(
    Object.values(filesByPath).map((file) => [file.path, createFileIndexEntry(file)]),
  );
  ...
  return {
    ...
    repository: {
      ...
      loadFile(filePath) {
        ...
        if (!file) {
          throw new Error(
            `File "${filePath}" is not declared in repository ${repository.projectRoot}`,
          );
        }
...
function selectDiffSubject(
  repository: HushV3Repository,
  options: Pick<DiffOptions, "target" | "bundle">,
): DiffSelection {
  ...
  if (options.bundle) {
    if (!repository.manifest.bundles?.[options.bundle]) {
      throw new Error(
        `Bundle "${options.bundle}" not found. Available bundles: ${
          Object.keys(repository.manifest.bundles ?? {})
            .sort()
            .join(", ") || "(none)"
        }`,
      );
...
function compareFileState(
  ...
  const filePaths = Array.from(
    new Set([...currentResolution.files, ...previousResolution.files]),
  ).sort();
  const lines: string[] = [];
...
function compareResolvedNodes(
  ...
  const logicalPaths = Array.from(
    new Set([...Object.keys(currentNodes), ...Object.keys(previousNodes)]),
  ).sort();
  const lines: string[] = [];
...
export async function diffCommand(ctx: HushContext, options: DiffOptions): Promise<void> {
  ...
  const previousResolution = resolveSelection(
    ctx,
    selection,
  ...
  const fileChanges = compareFileState(
    repository,
    historical.repository,
    currentResolution,
    previousResolution,
  ...
  if (fileChanges.length === 0 && valueChanges.length === 0) {
    ...
    lines.push(
      pc.green(
        `No redacted changes between current state and ${ref} for ${selection.kind} ${selection.name}.`,
      ),
    );
  }
