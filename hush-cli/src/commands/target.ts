...
async function handleAdd(ctx: HushContext, options: TargetAddOptions): Promise<void> {
  ...
  const command = {
    name: "target",
    args: ["add", options.name, "--bundle", bundle, "--format", options.format],
  };
  ...
  if (existingTarget) {
    ...
    throw new Error(
      `Target "${options.name}" already exists with different configuration. No target was changed. Remove it explicitly before replacing it.`,
    );
  }
...
async function handleRemove(ctx: HushContext, options: TargetRemoveOptions): Promise<void> {
  ...
  if (!targets[options.name]) {
    throw new Error(
      withSuggestion(
        `Target "${options.name}" was not found. Nothing was removed.`,
        options.name,
        Object.keys(targets),
      ),
    );
