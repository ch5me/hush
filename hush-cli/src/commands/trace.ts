...
function getBundlesReferencingFiles(
...
function formatNotSelectedDiagnosis(
  targetBundle: string | undefined,
  filePaths: readonly string[],
  candidateBundles: readonly string[],
): string {
  const fileLabel = filePaths.join(", ");
  const bundleLabel =
    candidateBundles.length > 0
      ? candidateBundles.join(", ")
      : "(no bundle directly references matched file)";
...
export async function traceCommand(ctx: HushContext, options: TraceOptions): Promise<void> {
  ...
  const machineLocalMatches = Object.keys(
    loadMachineLocalOverrides(ctx, options.store)?.entries ?? {},
  )
    .filter((logicalPath) => matchLogicalPath(options.key, logicalPath))
    .sort();
  const allMatchedLogicalPaths = Array.from(
    new Set([...matchedFiles.flatMap((entry) => entry.matches), ...machineLocalMatches]),
  ).sort();
  ...
  for (const entry of matchedFiles) {
    ...
    lines.push(
      `  ${pc.cyan(entry.file.path)} ${pc.dim(`(${status}; ${formatReaders(entry.file)})`)}`,
    );
  ...
  for (const targetName of targetNames) {
    ...
    try {
      ...
      if (matchedNodes.length === 0) {
        ...
        const diagnosis = formatNotSelectedDiagnosis(
          target?.bundle,
          matchedFilePaths,
          candidateBundles,
        );
      ...
      for (const [logicalPath, node] of matchedNodes.sort(([left], [right]) =>
      ...
      for (const filePath of extractUnreadableFilePaths(message)) {
        ...
        lines.push(
          `    ${pc.yellow(filePath)}${file ? ` ${pc.dim(`(${formatReaders(file)})`)}` : ""}`,
        );
      }
      targetResults.push({
        target: targetName,
  ...
  if (compactMode) {
    ...
    if (compactResults.length === 0) {
      ...
      for (const record of compactResults.sort(
        (left, right) =>
