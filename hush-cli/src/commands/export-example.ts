...
function toSafeEnvVar(
  path: string,
  entry: HushTargetResolution["values"][string]["entry"],
): EnvVar | null {
  if (entry.sensitive) {
    return null;
  }
...
function selectExportSubject(
  repository: ReturnType<typeof requireV3Repository>,
  options: Pick<ExportExampleOptions, "target" | "bundle">,
): ExportSelection {
...
function formatArtifactExample(
  node: HushBundleResolution["artifacts"][string],
  safeEnvVars: EnvVar[],
): string {
...
function createBundleExampleOutput(
  ...
  const artifactEntries = Object.entries(resolution.artifacts).sort(([left], [right]) =>
...
function createTargetExampleOutput(
  selection: ExportSelection & { kind: "target" },
  ...
  lines.push(
    `Format: ${pc.dim(selection.target.format)}${selection.target.mode ? pc.dim(` (${selection.target.mode})`) : ""}`,
  );
  lines.push(`Active identity: ${pc.green(resolution.identity)}`);
  lines.push(`Protected values omitted: ${pc.cyan(String(omittedValues))}`);
  lines.push("");
...
export async function exportExampleCommand(
  ctx: HushContext,
  options: ExportExampleOptions,
): Promise<void> {
  ...
  if (selection.kind === "bundle") {
    ...
    if (options.json)
      writeJsonSuccess(ctx, "export-example", { selection, output, writtenTo: filePath ?? null });
