...
function filterLegacyVars(vars: EnvVar[], target: LegacyTarget): EnvVar[] {
  ...
  if (target.include && target.include.length > 0) {
    filtered = filtered.filter((variable) =>
      target.include?.some((pattern) => matchesPattern(variable.key, pattern)),
    );
  }

  if (target.exclude && target.exclude.length > 0) {
    filtered = filtered.filter(
      (variable) => !target.exclude?.some((pattern) => matchesPattern(variable.key, pattern)),
    );
  }
...
function withSourcePresence(
  ctx: HushContext,
  root: string,
  sources: LegacySourceInventory[],
...
function getLocalOverridePath(store: StoreContext): string {
  return join(
    store.projectStateRoot ?? getProjectStatePaths(store).projectRoot,
    "user",
...
function ensureNoRootFileConflict(ctx: HushContext, root: string): void {
  ...
  if (ctx.fs.existsSync(hushPath) && !ctx.fs.statSync(hushPath).isDirectory()) {
    throw new Error(
      `Cannot migrate while ${hushPath} is a file. Remove or rename the legacy plaintext source before creating the v3 .hush/ directory.`,
    );
  }
}

function validateMigrationPrerequisites(
  ctx: HushContext,
  root: string,
  sources: LegacySourceInventory[],
): void {
  const hasLegacySourceInputs = sources.some(
    (source) => source.encryptedExists || source.plaintextExists,
  );
...
function repairGitignoreForV3(ctx: HushContext, root: string): string[] {
  ...
  const nonEmptyLines = keptLines.filter(
    (line, index, arr) => !(index === arr.length - 1 && line === ""),
  );
...
function buildManifest(
  ...
  return createManifestDocument({
    ...
    identities: {
      ...
      "member-local": {
        roles: ["member"],
        description: "Default member identity migrated from v2",
      },
      ci: { roles: ["ci"], description: "Default automation identity migrated from v2" },
    },
    fileIndex: Object.fromEntries(
      fileDocuments.map((document) => [document.path, createFileIndexEntry(document)]),
    ),
    bundles: {
      project: {
      ...
      ...Object.fromEntries(
        Object.entries(bundleFiles).map(([bundleName, filePath]) => [
          bundleName,
          {
            files: [{ path: filePath }],
          },
    ...
    metadata: {
      project:
        inventory.config.project ??
        getProjectIdentifier(inventory.projectRoot) ??
...
function printInventory(
  ...
  console.log(
    `  Legacy sources: ${sources.filter((source) => source.encryptedExists || source.plaintextExists).length}/${sources.length} source files found`,
  );
  console.log(`  Legacy targets: ${inventory.targets.length}`);
  console.log(`  Repo refs to review: ${references.length}`);
  for (const source of sources) {
    const locatedPath = source.encryptedExists
      ? source.encryptedPath
      : source.plaintextExists
        ? source.sourcePath
        : source.encryptedPath;
    const suffix = source.encryptedExists
      ? ""
      : source.plaintextExists
        ? " (plaintext)"
        : " (missing)";
...
function validateMigratedRepository(
  ctx: HushContext,
  store: StoreContext,
  targetNames: string[],
): void {
...
function cleanupMigratedLegacyFiles(
  ctx: HushContext,
  root: string,
...
export async function migrateCommand(ctx: HushContext, options: MigrateOptions): Promise<void> {
  ...
  if (v3Exists) {
    ...
    if (marker?.validated) {
      ctx.logger.log(
        pc.dim('Run "hush migrate --from v2 --cleanup" to remove any remaining legacy files.'),
      );
    }
    return;
  }
  ...
  try {
    ...
    for (const target of inventory.targets) {
      ...
      const developmentView = filterLegacyVars(
        interpolateVars(mergeVars(sharedVars, developmentVars, localVars)),
        target,
      );
      const productionView = filterLegacyVars(
        interpolateVars(mergeVars(sharedVars, productionVars, localVars)),
        target,
      );
      ...
      writeYamlDocument(
        ...
        root,
        keyIdentity,
        getV3EncryptedFilePath(root, developmentFilePath),
        developmentDocument,
      );
      ...
      if (productionView.length > 0) {
        ...
        writeYamlDocument(
          ...
          root,
          keyIdentity,
          getV3EncryptedFilePath(root, productionFilePath),
          productionDocument,
        );
    ...
    writeMarker(ctx, root, {
      ...
      legacyEncryptedFiles: sources
        .filter((source) => source.encryptedExists)
        .map((source) => source.encryptedPath),
      cleanupCandidates: [
  ...
  if (repairedGitignoreEntries.length > 0) {
    ctx.logger.log(
      pc.yellow(
        `Updated .gitignore to stop ignoring the v3 .hush/ repo (${repairedGitignoreEntries.join(", ")} removed; .hush-materialized/ kept ignored).`,
      ),
    );
  }
  ctx.logger.log(
