 ...
 */
function peekGlobalStoreCounts(
  globalRoot: string,
): { targetCount: number; bundleCount: number } | undefined {
...
export async function statusCommand(ctx: HushContext, options: StatusOptions): Promise<void> {
  ...
  try {
    ...
    const repositoryStatus = isV3RepositoryRoot(options.store.root)
      ? "ready"
      : (projectInfo?.repositoryKind ?? "missing");
    ...
    if (!isV3RepositoryRoot(options.store.root)) {
      ...
      ctx.logger.log(
        `Store: ${pc.cyan(options.store.mode)} ${pc.dim(`(${options.store.displayLabel})`)}`,
      );
      ...
      ctx.logger.log(
        pc.dim('Migrate with "hush migrate --from v2" before relying on normal command flows.'),
      );
      return;
    }

    const authority = loadV3Repository(options.store.root, {
    ...
    const globalCounts =
      options.store.mode !== "global" ? peekGlobalStoreCounts(GLOBAL_STORE_ROOT) : null;
    ...
    ctx.logger.log(
      `Active identity: ${activeIdentity ? pc.green(activeIdentity) : pc.yellow("(not set)")}`,
    );
    ctx.logger.log("");
    ...
    if (globalCounts !== null) {
      ...
      if (globalExists && globalCounts) {
        ctx.logger.log(
          `  ${globalDisplayPath}: ${pc.cyan(`${globalCounts.targetCount} target(s), ${globalCounts.bundleCount} bundle(s)`)}`,
        );
      } else {
      ...
      ctx.logger.log(
        pc.dim(
          `  Not auto-inherited — compose via \`hush import add --source-root ${globalDisplayPath}\` or use \`hush --root ${globalDisplayPath} <cmd>\` for one-off access.`,
        ),
      );
    }
    ...
    ctx.logger.log(
      `  active identity path: ${pc.dim(statePaths.activeIdentityPath)} ${pc.dim(`(${formatStateHealth(ctx, statePaths.activeIdentityPath)})`)}`,
    );
    ctx.logger.log(
      `  audit log path: ${pc.dim(statePaths.auditLogPath)} ${pc.dim(`(${formatStateHealth(ctx, statePaths.auditLogPath)})`)}`,
    );
  } catch (error) {
