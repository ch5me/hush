...
describe("bootstrap/config/init task 6", () => {
  ...
  it("bootstraps a v3 repository layout and active identity state", async () => {
    ...
    expect(nodeFs.existsSync(join(projectRoot, ".hush/files/env/project/shared.encrypted"))).toBe(
      true,
    );
    ...
    expect(Object.keys(repository.manifest.identities)).toEqual([
      "owner-local",
      "member-local",
      "ci",
    ]);
  ...
  it("parses bundle subcommands and name/files flags through cli argument parsing", () => {
    const addParsed = parseArgs([
      ...
      "my-bundle",
      "--files",
      "env/project/shared,env/project/production",
      "--json",
    ]);
    ...
    const removeFileParsed = parseArgs([
      "bundle",
      "remove-file",
      "my-bundle",
      "env/project/staging",
