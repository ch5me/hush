...
describe("keysRecoverFromVercel", () => {
  ...
  it("throws when SOPS_AGE_KEY is absent from the Vercel project", async () => {
    ...
    ctx.network = {
      fetch: vi
        ...
        .mockResolvedValue(createJsonResponse({ envs: [{ key: "OTHER", value: "x" }] }, 200)),
    };
  ...
  it("throws a VercelPushError on non-2xx response", async () => {
    ...
    ctx.network = {
      fetch: vi
        .fn<typeof globalThis.fetch>()
        .mockResolvedValue(createJsonResponse({ error: { message: "Unauthorized" } }, 401)),
    };
  ...
  it("falls back to VERCEL_TOKEN env var when no explicit token is passed", async () => {
    ...
    const calledInit = (ctx.network!.fetch as ReturnType<typeof vi.fn>).mock
  ...
  it("overwrites an existing key when force=true", async () => {
    ...
    ctx.network = {
      fetch: vi
        ...
        .mockResolvedValue(
          createJsonResponse(
            { envs: [{ key: "SOPS_AGE_KEY", value: "AGE-SECRET-KEY-FORCED" }] },
            200,
          ),
        ),
    };
...
describe("keysCommand pull routing", () => {
  ...
  it("exits with error when --from is missing", async () => {
    ...
    await expect(keysCommand(ctx, { store: createStore(), subcommand: "pull" })).rejects.toThrow(
      "Process exit: 1",
    );
