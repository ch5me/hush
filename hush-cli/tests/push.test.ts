            ...
            targets: [
              {
                name: "pages",
                path: "./apps/pages",
                push_to: { type: "cloudflare-pages", project: "docs" },
              },
            ],
          },
        },
  ...
  it("collects non-2xx failures without leaking secret values", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        createJsonResponse(
          { error: { message: "No access to project" } },
...
describe("pushCommand wrangler-env", () => {
  ...
  it("shows env label in dry-run output when wranglerEnv is set", async () => {
    ...
    const logCalls = (ctx.logger.log as ReturnType<typeof vi.fn>).mock.calls.map(
      (c) => c[0] as string,
    );
