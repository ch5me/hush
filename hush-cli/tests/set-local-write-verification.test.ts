...
import {
  getMachineLocalOverridePath,
  loadMachineLocalOverrides,
} from "../src/commands/v3-command-helpers.js";
import {
  decrypt,
  decryptYaml,
  encrypt,
...
function normalizeYaml(content: string): string {
  ...
  const indent = lines
    ...
    .reduce<number>(
      (smallest, line) => Math.min(smallest, line.match(/^\s*/)?.[0].length ?? 0),
...
function createContext(root: string) {
  ...
  const ctx: HushContext = {
    ...
    sops: {
      ...
      decryptYaml: vi.fn((p: string, o?: { root?: string; keyIdentity?: string }) =>
        decryptYaml(p, o),
      ),
      encrypt: vi.fn((i: string, o: string, opt?: { root?: string; keyIdentity?: string }) =>
        encrypt(i, o, opt),
      ),
      encryptYaml: vi.fn((i: string, o: string, opt?: { root?: string; keyIdentity?: string }) =>
        encryptYaml(i, o, opt),
      ),
      encryptYamlContent: vi.fn(
        (c: string, o: string, opt?: { root?: string; keyIdentity?: string }) =>
          encryptYamlContent(c, o, opt),
      ),
      edit: vi.fn(),
 ...
 */
async function resolvesThroughRuntime(
  ctx: HushContext,
  store: StoreContext,
  key: string,
): Promise<boolean> {
...
describe("set writes to the machine-local store are persisted and verified", () => {
  ...
  it("set --file local (alias form) resolves back through the runtime target view", async () => {
    ...
    await setCommand(ctx, {
      store,
      file: "local",
      key: "LOCAL_ALIAS_FORM",
      value: "value-alias-form",
    });
  ...
  it("set --repo-local resolves back in global store mode", async () => {
    ...
    await setCommand(ctx, {
      store,
      repoLocal: true,
      key: "GLOBAL_LOCAL_KEY",
      value: "value-global-local",
    });
  ...
  it("warns instead of reporting a clean success when the written file is not selected by the target", async () => {
    ...
    await setCommand(ctx, {
      ...
      key: "UNBUNDLED_KEY",
      value: "value-unbundled",
    });

    const warnings = stripAnsi(
      logger.warn.mock.calls.map(([message]) => String(message)).join("\n"),
    );
  ...
  it("fails loud instead of reporting success when a machine-local write does not persist", async () => {
    ...
    ctx.sops.encryptYamlContent = vi.fn(
  ...
  it("fails loud instead of reporting success when a repository write does not persist", async () => {
    ...
    await expect(
      setCommand(ctx, {
        store,
        file: "shared",
        key: "DROPPED_SHARED_KEY",
        value: "value-dropped-shared",
      }),
    ).rejects.toThrow(
      /Write verification failed for DROPPED_SHARED_KEY in env\/project\/shared \(repository\)/,
    );
...
describe("storage class is named by the path, never by manifest state", () => {
  ...
  it("keeps writing a declared env/project/local to the repository file", async () => {
    ...
    const repositoryFile = decryptYaml(
      join(root, ".hush", "files", "env", "project", "local.encrypted"),
      { root, keyIdentity: root },
    );
  ...
  it("does not let a declared env/project/local collide with the machine-local store", async () => {
    ...
    const machineLocal = decryptYaml(getMachineLocalOverridePath(store), {
      root,
      keyIdentity: root,
    });
  ...
  it("reports a sops preflight timeout as itself, not as a corrupt machine-local override file", () => {
    ...
    expect(() => loadMachineLocalOverrides(ctxWithWedgedSops, store)).toThrow(
      SopsPreflightTimeoutError,
    );
    try {
