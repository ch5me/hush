...
describe("mutation spelling feedback", () => {
  it("suggests one unambiguous close spelling without changing it", () => {
    expect(
      suggestKnownName("env/project/produciton", ["env/project/shared", "env/project/production"]),
  ...
  it("states the suggestion as guidance, not an applied correction", () => {
    expect(
      ...
      'File "env/project/produciton" is not declared. Nothing was changed. Did you mean "env/project/production"?',
    );
  });
});
