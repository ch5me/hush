...
function fakeSopsManifest(targets: string[], bundles: string[]): string {
  ...
  for (const t of targets) {
    targetObj[t] = {
      bundle: "ENC[AES256_GCM,data:fake,iv:fake,tag:fake,type:str]",
      format: "ENC[AES256_GCM,data:fake,iv:fake,tag:fake,type:str]",
    };
  }
...
function setupGlobalStore(globalRoot: string, targets: string[], bundles: string[]): void {
  ...
  writeFileSync(
    join(manifestDir, "manifest.encrypted"),
    fakeSopsManifest(targets, bundles),
    "utf-8",
  );
}
