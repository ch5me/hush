...
function writeLegacyRepo(
  root: string,
  overrides?: { pagesTarget?: boolean; plaintextLocal?: string; sharedOverrideContent?: string },
): void {
...
${overrides?.pagesTarget ? "  - name: pages\n    path: ./apps/pages\n    format: wrangler\n    push_to:\n      type: cloudflare-pages\n      project: docs\n" : ""}`.replace(
  ...
  writeEncryptedDotenvFile(
    root,
    join(root, ".env.encrypted"),
    overrides?.sharedOverrideContent ??
      "DATABASE_URL=postgres://db\nNEXT_PUBLIC_API_URL=https://example.com",
  );
  writeEncryptedDotenvFile(
    root,
    join(root, ".env.development.encrypted"),
    "DEBUG=true\nAPI_BASE=http://localhost:3000",
  );
  writeEncryptedDotenvFile(
    root,
    join(root, ".env.production.encrypted"),
  ...
  nodeFs.writeFileSync(
    join(root, "README.md"),
    "Use `hush run -- npm start` after migration.\n",
    "utf-8",
  );
}
...
describe("migrateCommand v2 -> v3", () => {
  ...
  it("converts a legacy repo to v3 state and writes machine-local overrides", async () => {
    ...
    expect(
      nodeFs.existsSync(join(root, ".hush", "files", "env", "project", "development.encrypted")),
    ).toBe(true);
    expect(
      nodeFs.existsSync(join(root, ".hush", "files", "env", "project", "production.encrypted")),
    ).toBe(true);
  ...
  it("short-circuits safely when the repo is already migrated", async () => {
    ...
    expect(readDecryptedYamlFile(root, join(root, ".hush", "manifest.encrypted"))).toBe(
      firstManifest,
    );
    expect(ctx.logger.log).toHaveBeenCalledWith(
      expect.stringMatching(/already has \.hush\/ v3 state/i),
    );
  }, 60000);
  ...
  it("fails without leaving mixed state when .hush is still a legacy file", async () => {
    ...
    await expect(
      migrateCommand(ctx, { root, dryRun: false, from: "v2", cleanup: false }),
  ...
  it("fails clearly on keyless template repos and recommends hush bootstrap", async () => {
    ...
    await expect(
      migrateCommand(ctx, { root, dryRun: false, from: "v2", cleanup: false }),
    ).rejects.toThrow(/hush bootstrap/i);
    expect(nodeFs.existsSync(join(root, ".hush", "manifest.encrypted"))).toBe(false);
  });
  ...
  it("migrates plaintext local overrides and resolves self-referential placeholders before v3 validation", async () => {
    ...
    const runtimeFile = readDecryptedYamlFile(
      root,
      join(root, ".hush", "files", "env", "targets", "api", "runtime.encrypted"),
    );
  ...
  it("fails migration with a clear unresolved interpolation error before v3 materialization crashes", async () => {
    ...
    await expect(
      migrateCommand(ctx, { root, dryRun: false, from: "v2", cleanup: false }),
    ).rejects.toThrow(/Legacy interpolation placeholders remained unresolved/i);
    expect(nodeFs.existsSync(join(root, ".hush", "manifest.encrypted"))).toBe(false);
  });
