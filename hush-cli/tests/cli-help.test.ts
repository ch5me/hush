...
describe("agent-first command help", () => {
  ...
  it("supports command help end to end with diagnostics kept off stdout", () => {
    const result = spawnSync("bun", ["src/cli.ts", "set", "--help"], {
      cwd: process.cwd(),
      encoding: "utf8",
      env: { ...process.env, HUSH_CLI_ENTRYPOINT: "1" },
    });
...
describe("central CLI errors", () => {
  const run = (...args: string[]) =>
    spawnSync("bun", ["src/cli.ts", ...args], {
      cwd: process.cwd(),
      encoding: "utf8",
      env: { ...process.env, HUSH_CLI_ENTRYPOINT: "1", HUSH_NO_UPDATE_CHECK: "1" },
    });
  ...
  it("returns structured unknown-option and subcommand suggestions", () => {
    ...
    expect(JSON.parse(option.stderr).error).toMatchObject({
      code: "UNKNOWN_OPTION",
      rejectedInput: "--fiel",
    });
