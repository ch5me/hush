...
function verifySourceBin() {
  ...
  assertSuccess('source version check', process.execPath, ['./bin/hush.js', '--version'], versionResult);

  if (versionResult.stdout.trim() !== pkg.version) {
    fail(`source version output mismatch: expected ${pkg.version}, got ${JSON.stringify(versionResult.stdout.trim())}`);
  }
...
function verifyInstalledTarball(tarballPath) {
  ...
  try {
    ...
    const installedPackageBinPath = join(installRoot, 'node_modules', '@chriscode', 'hush', 'bin', 'hush.js');
    ...
    if (versionResult.stdout.trim() !== pkg.version) {
      fail(`installed version output mismatch: expected ${pkg.version}, got ${JSON.stringify(versionResult.stdout.trim())}`);
    }
