...
# Emit shell-safe export statements instead of summary text
hush materialize -t runtime --format shell-export
...
| `--format <dotenv\|shell-export>` | Render env output as summary text with persisted dotenv target default, or shell-safe `export KEY=...` lines |
| `--compact-json` | Emit minimal JSON with status plus artifact paths only |
| `--include-provenance` | Add provenance and `resolvedFrom` details to JSON output |
...
`--json` returns metadata only — never raw secret contents.

- Default JSON keeps existing full metadata like `repositoryRoot`, `files`, `logicalPaths`, hashes, and path details.
- `--compact-json` trims that down to `status`, selection info, and persisted artifact paths.
- `--include-provenance` adds `provenance` and `resolvedFrom` details to the full JSON payload.

This makes it suitable for CI pipelines that need either a tiny handoff surface or a richer audit/debug surface without logging private values.
...
# Verify length before writing (useful for TTY input)
hush set API_KEY --show-length
```
...
| `--show-length` | Show the length of the entered value before writing (useful for TTY input verification) |
...
| Option | Description |
|--------|-------------|
| `--from <file-path>` | Source encrypted v3 file path, such as `env/project/shared` |
...
hush resolve runtime --compact

# Filter to one key
hush resolve runtime --only DATABASE_URL

# Minimal machine-readable output
hush resolve runtime --json-compact --only DATABASE_URL
```
...
| `--compact` | Output only key, source file, target, and precedence |
| `--only <key>` | Filter values and artifacts to one leaf key or full logical path |
| `--json-compact` | Output minimal machine-readable records: `key`, `source`, `target`, `precedence` |
...
`--compact` keeps terminal output short for quick checks. `--json-compact` keeps automation payloads stable and minimal when you only need key reachability.
...
# Minimal human-readable output
hush trace RESEND_API_KEY --compact

# Minimal machine-readable output
hush trace RESEND_API_KEY --json-compact
```
...
| `--compact` | Output only key, source file, target, and precedence for resolved targets |
| `--json-compact` | Output minimal machine-readable records: `key`, `source`, `target`, `precedence` |
...
Use `--compact` or `--json-compact` when you only need resolved target hits, not full repository and denial diagnostics.
