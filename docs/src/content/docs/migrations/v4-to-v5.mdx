...
> Historical shipped migration note. This page documents the older file-naming transition into `.hush*.encrypted`. The current repository migration story is the big-bang `hush migrate --from v2 [--cleanup]` flow into `.hush/` v3 repository state.
...
2. **Run the migrate command**
   ...
   ```bash
   npx @chriscode/hush migrate --dry-run    # Preview changes
   npx @chriscode/hush migrate              # Apply changes
   ```
...
5. **Verify**
   ...
   ```bash
   npx @chriscode/hush status    # Should show .hush.encrypted files
   npx @chriscode/hush inspect   # Should show your secrets
   ```
...
Steps:
1. Run `npx @chriscode/hush migrate --dry-run` to preview
2. Run `npx @chriscode/hush migrate` to apply
3. Update git (add new files, remove old)
4. Update .gitignore
5. Verify with `npx @chriscode/hush status`
```
