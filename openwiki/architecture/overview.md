...
- **INT-06 sentinels:** Cross-boundary failure and security acceptance is fixture/synthetic-sentinel only (`MISS-INT06-LIVE`). Critical security/identity/no-fallback sentinels cover EXE-02, DEV-07, SEC-05, FF-04, OBS-01, and UI-02 as opaque refs. Untested boundaries narrow or block; post-hoc criteria changes are refused. `live_run` stays false and `ready_for_live` stays `blocked_by_D06`. See `.ch5/int-06-sentinels.json` and `docs/INT-06-SENTINELS.md`.
